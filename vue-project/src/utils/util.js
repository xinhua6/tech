...
export function treeEach (data = [], fn, field = 'children', depth = 0, parent) {
  for (let i = 0, len = data.length; i < len; i++) {
    const item = data[i]
    fn(item, i, depth, parent, data)
    const children = item[field]
    if (children && children.length) {
      treeEach(children, fn, field, depth + 1, item)
    }
  }
}
...
* 查找树节点中所有子节点
* */
export function findSonList (json, arr) {
  ...
  for (let i = 0; i < json.children.length; i++) {
    debugger
    let sonList = JSON.parse(JSON.stringify(json.children[i]))
    if (sonList.children.length === 0) {
      arr.push(sonList)
    } else {
      findSonList(sonList, arr)
    }
  }
  return arr
}
