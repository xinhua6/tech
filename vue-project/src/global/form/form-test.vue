<template>
  <form class="c-form"
        :class="{
           'c-form-view':isView,
          'c-form-disabled': formDisabled,
          'c-form-top': labelPosition === 'top',
          'c-form-left':labelPosition === 'left'
    }">
    <item v-for="item in items" :key="item.name + item.label">
...
<script>
  ...
  export default {
    ...
    render (h, context) {
      let child = null
      if (this.isView) {
        child = this.renderRow(h)
      } else {
        child = this.createdChild(h)
      }
      return h('form', {
        staticClass: 'c-form',
        class: {
    ...
    methods: {
      ...
      // 获取修改的值
      getChange () {
        let obj = {}; let oval = this.$_cloneData; let nval = this.data
        for (let key in oval) {
      ...
      renderItem (h, item, width) {
        ...
        let data = this.data; let key = item.name
