...
import Events, { bind, mouseMoveUp, bindTouch } from './event'
...
function selectorSet (multiple, ri, ci, indexesUpdated = true, moving = false) {
  ...
  events.emit('afterSelection', range.sri, range.sci, range.eri, range.eci)
...
function overlayerMousemove (evt) {
  ...
  const {
    rowResizer, colResizer, tableEl, data, hoverCell
  } = this
  const { rows, cols } = data
  if (offsetX > cols.indexWidth && offsetY > rows.height) {
    rowResizer.hide()
    colResizer.hide()
    if (hoverCell.rowIndex !== -2) {
      this.events.emit('afterCellOut', Object.assign({}, hoverCell))
      hoverCell.rowIndex = -2
      hoverCell.colIndex = -2
    }
    return
  }
  ...
  if (hoverCell.rowIndex !== cRect.ri || hoverCell.colIndex !== cRect.ci) {
    if (hoverCell.rowIndex !== -2) {
      this.events.emit('afterCellOut', Object.assign({}, hoverCell))
    }
    hoverCell.rowIndex = cRect.ri
    hoverCell.colIndex = cRect.ci
    hoverCell.offsetLeft = cRect.left
    hoverCell.offetTop = cRect.top
    hoverCell.width = cRect.width
    hoverCell.height = cRect.height
    this.events.emit('afterCellOver', Object.assign({}, hoverCell))
  }
}
...
function sheetInitEvents () {
  ...
  const listeners = data.settings.events
  if (typeof listeners === 'object') {
    Object.keys(listeners).forEach(key => {
      events.on(key, listeners[key])
    })
  }
}
