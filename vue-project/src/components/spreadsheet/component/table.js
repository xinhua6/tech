...
function renderFixedHeaders (type, viewRange, w, h, tx, ty) {
  ...
  // y-header-text
  const yHeadrTextRender = data.settings.renderRowHeader ? data.settings.renderRowHeader : function (index, text) {
    return text
  }
  if (type === 'all' || type === 'left') {
    data.rowEach(viewRange.sri, viewRange.eri, (i, y1, rowHeight) => {
      ...
      draw.fillText(yHeadrTextRender(ii, ii + 1), w / 2, y + (rowHeight / 2))
    })
    draw.line([0, sumHeight + nty], [w, sumHeight + nty])
    draw.line([w, nty], [w, sumHeight + nty])
  }
  // x-header-text
  const xHeadrTextRender = data.settings.renderColHeader ? data.settings.renderColHeader : function (index, text) {
    return text
  }
  if (type === 'all' || type === 'top') {
    data.colEach(viewRange.sci, viewRange.eci, (i, x1, colWidth) => {
      ...
      draw.fillText(xHeadrTextRender(ii, stringAt(ii)), x + (colWidth / 2), h / 2)
    })
